...
export function StructuralBeams({ color = "#ffffff", opacity = 0.15, speed = 4, size = 1 }: Partial<PatternConfig>) {
  // size controls column count: fine=more columns, coarse=fewer
  const cols = Math.max(3, Math.round(8 / size))
  const rows = Math.max(2, Math.round(5 / size))
    ...
    >
      ...
      {[...Array(cols)].map((_, i) => (
        <div
          ...
          style={{ left: `${colStep * (i + 1)}%`, opacity }}
        />
      ))}
      {/* Horizontal beams */}
      {[...Array(rows)].map((_, i) => (
